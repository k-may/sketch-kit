...
export class MainView extends BaseView {
    ...
    initialize() {
        ...
        LoadingUtils.LoadJSON('config.json').then(start).catch(e => {
            //fallback for older versions
            LoadingUtils.LoadJSON('data/config.json').then(start)
        });
    }
     ...
     */
    draw({time, deltaTime}) {
        if (this._sketch) this._sketch.draw({time, deltaTime});

        //update smooth scroll
        if (this._expanderEl) {
            this._scrollTo +=
                (this._destScrollTo - this._scrollTo) * 0.1;
            var sP = 1.0 - (this._expanderHeight - window.innerHeight - this._scrollTo) / (this._expanderHeight - window.innerHeight);
